...
// excelSheetName is the worksheet name used for Excel import/export - matches the
// Python/PHP/Java implementations.
const excelSheetName = "Todo Items"
...
// ExportExcel renders every todo item as an .xlsx workbook. Mirrors export_excel in the
// Python/PHP/Java implementations (same sheet name, header row and column order as CSV).
func (s *todoItemService) ExportExcel() ([]byte, error) {
	items, err := s.repo.FindAllItems()
	if err != nil {
		return nil, err
	}
	...
	sheetIndex, err := f.NewSheet(excelSheetName)
	if err != nil {
		return nil, err
	}
	...
	header := make([]interface{}, len(csvFieldNames))
	for i, name := range csvFieldNames {
		header[i] = name
	}
	if err := f.SetSheetRow(excelSheetName, "A1", &header); err != nil {
		return nil, err
	}
	...
	for i, item := range items {
		...
		if err := f.SetSheetRow(excelSheetName, fmt.Sprintf("A%d", i+2), &row); err != nil {
			return nil, err
		}
	}
